...
schedule = RedisSchedule(queue_name)
huey = Huey(queue, schedule=schedule)

res_queue_name = 'redis-queue-2'
res_queue = RedisQueue(res_queue_name)
res_store = RedisDataStore(res_queue_name)
...
class RedisTestCase(unittest.TestCase):
    ...
    def test_schedule(self):
        ...
        r_add.schedule(args=('k2', 'v2'), eta=dt)
        self.assertEqual(len(queue), 1)
        task = huey.dequeue()
        self.assertEqual(task.execute_time, local_to_utc(dt))

        r_add.schedule(args=('k3', 'v3'), eta=dt, convert_utc=False)
        self.assertEqual(len(queue), 1)
        task = huey.dequeue()
    ...
    def test_error_raised(self):
        ...
        # error
        self.assertRaises(ScampException, huey.execute, task)
    ...
    def test_self_awareness(self):
        ...
        task = huey.dequeue()
        huey.execute(task)
        self.assertEqual(last_executed_task_class.pop(), "queuecmd_r_self_aware")
    ...
    def test_call_local(self):
        ...
        r_add.call_local('nugget', 'green')

        self.assertEqual(len(queue), 0)
        self.assertEqual(state['nugget'], 'green')

    @unittest.skip('revoking not working properly w redis backend?')
    ...
    def test_task_store(self):
        ...
        r_add2.schedule(args=('k', 'v'), eta=dt1, convert_utc=False)
        task1 = res_huey.dequeue()

        r_add2.schedule(args=('k2', 'v2'), eta=dt2, convert_utc=False)
    ...
    def test_ready_to_run_method(self):
        ...
        r_add2.schedule(args=('k', 'v'), eta=dt1)
        task1 = res_huey.dequeue()

        r_add2.schedule(args=('k2', 'v2'), eta=dt2)
        task2 = res_huey.dequeue()

        r_add2('k3', 'v3')
        task3 = res_huey.dequeue()

        r_add2.schedule(args=('k4', 'v4'), task_id='test_task_id')
    ...
    def test_task_delay(self):
        ...
        r_add2.schedule(args=('k', 'v'), delay=60)
        task1 = res_huey.dequeue()

        r_add2.schedule(args=('k2', 'v2'), delay=600)
        task2 = res_huey.dequeue()
